import z from "zod";
import type { DbDocData, DbDocType, DocSnapType, Infer, Parsed, ParsedPartial } from "../models/base/base.types.js";
import { ConflictError, DocumentNotFoundError, ValidationError } from "../models/errors/Errors.js";

export const zEnforceNonEmptyStr = (fieldName?: string) => z.string().trim().min(1, { error: `${fieldName || 'Field'} must be a non-empty string` })
    ...
    const cleaned = opts?.omit
        ? Object.fromEntries(
            Object.entries(data).filter(([k]) =>
                !(opts.omit as readonly string[]).includes(k)
            )
        )
        : data
...
export function isFirestoreError(
    e: unknown
): e is { code: unknown; message?: unknown } {
    return (
        typeof e === "object" &&
        e !== null &&
        "code" in e
