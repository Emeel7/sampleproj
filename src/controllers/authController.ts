...
import { dbCreateNewUser, dbLoginUser, dbUpdateUserSessionToken } from "../models/User/Users.js"
import { parseLoginAttempt, parseNewUserDetails, parseUserId } from "./parsers/userRequestParsers.js"
...
export const loginUser = async (req: Request, res: Response) => {
    const { method, data: { identifier, password } } = parseLoginAttempt(req.body)

    const user = await dbLoginUser(method, identifier, password)

    return res
        .cookie(...shortCookieArgs(user.sessionToken))
        .sendStatus(204)
}
